    ...
    s"should have the workspace-related environment variables set" in { clusterFixture =>
      withWebDriver { implicit driver =>
        withNewNotebook(clusterFixture.cluster, RKernel) { notebookPage =>
          ...
          notebookPage.executeCell("Sys.getenv('WORKSPACE_NAME')").get shouldBe "'jupyter-user'" // TODO: change to "jupyter-user" once https://github.com/DataBiosphere/terra-docker/pull/49 is merged

...
final class NotebookCustomizationSpec extends GPAllocFixtureSpec with ParallelTestExecution with NotebookTestUtils {
  ...
  "NotebookCustomizationSpec" - {
    ...
    "should set environment variables for old image" in { billingProject =>
      ...
      withNewCluster(billingProject, request = defaultClusterRequest) { cluster =>
        withWebDriver { implicit driver =>
          withNewNotebook(cluster, Python3) { notebookPage =>
            notebookPage.executeCell("! echo $GOOGLE_PROJECT").get shouldBe billingProject
            notebookPage.executeCell("! echo $WORKSPACE_NAMESPACE").get shouldBe billingProject

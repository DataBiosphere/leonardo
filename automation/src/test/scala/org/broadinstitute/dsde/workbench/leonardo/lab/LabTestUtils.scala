...
trait LabTestUtils extends LeonardoTestUtils {
  ...
  def withLabLauncherPage[T](cluster: ClusterFixture)(testCode: LabLauncherPage => T)(implicit webDriver: WebDriver, token: AuthToken): T = {
    val labLauncherPage = lab.Lab.get(cluster.googleProject, cluster.clusterName)
    testCode(labLauncherPage.open)
  }

  def withNewLabNotebook[T](cluster: ClusterFixture, kernel: LabKernel = lab.Python2, timeout: FiniteDuration = 2.minutes)(testCode: LabNotebookPage => T)(implicit webDriver: WebDriver, token: AuthToken): T = {

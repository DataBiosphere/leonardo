...
import org.broadinstitute.dsde.workbench.leonardo.algebra.MockGKEService
...
import org.broadinstitute.dsde.workbench.leonardo.algebra.{FakeGoogleSubcriber, GKEAlgebra, MockSamDAO, SamDAO}
import org.broadinstitute.dsde.workbench.leonardo.db.{clusterQuery, kubernetesClusterQuery, DbReference, TestComponent}
import org.broadinstitute.dsde.workbench.leonardo.subscriber.NonLeoMessage.{
...
class NonLeoMessageSubscriberSpec extends AnyFlatSpec with LeonardoTestSuite with TestComponent {
  ...
  it should "handle cryptomining message" in isolatedDbTest { implicit dbRef =>
    for {
  ...
  it should "handle DeleteKubernetesClusterMessage" in isolatedDbTest { implicit dbRef =>
  ...
  it should "handle DeleteNodepoolMessage" in isolatedDbTest { implicit dbRef =>
    val subscriber = makeSubscribler()
  ...
  )(implicit dbRef: DbReference[IO]): NonLeoMessageSubscriber[IO] = {

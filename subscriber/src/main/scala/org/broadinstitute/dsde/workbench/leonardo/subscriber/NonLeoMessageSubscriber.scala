...
import org.broadinstitute.dsde.workbench.leonardo.subscriber.NonLeoMessageSubscriber.cryptominingUserMessageEncoder
...
class NonLeoMessageSubscriber[F[_]: Timer](gkeAlg: GKEAlgebra[F],
  ...
  val process: Stream[F, Unit] = Stream.eval(subscriber.start) ++ subscriber.messages
  ...
  private[subscriber] def messageHandler(event: Event[NonLeoMessage]): F[Unit] = {
  ...
  private[subscriber] def messageResponder(
  ...
  private[subscriber] def handleDeleteKubernetesClusterMessage(msg: DeleteKubernetesClusterMessage)(
  ...
  private[subscriber] def handleCryptoMiningMessage(
  ...
  private[subscriber] def handleDeleteNodepoolMessage(

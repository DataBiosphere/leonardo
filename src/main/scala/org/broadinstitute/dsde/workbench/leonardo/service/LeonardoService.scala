  ...
  def handleResizingCluster(existingCluster: Cluster, machineConfigOpt: Option[MachineConfig]): Future[Unit] = {
    machineConfigOpt match {
      case Some(machineConfig) =>
        for {
          _ <- gdDAO.resizeCluster(existingCluster.googleProject, existingCluster.clusterName, machineConfig.numberOfWorkers, machineConfig.numberOfPreemptibleWorkers, existingCluster.serviceAccountInfo.clusterServiceAccount)

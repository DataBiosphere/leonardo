...
trait ClusterComponent extends LeoComponent {
  ...
  class ClusterTable(tag: Tag) extends Table[ClusterRecord](tag, "CLUSTER") {
    ...
    def * = (clusterId, clusterName, googleProject, googleServiceAccount, googleBucket, operationName, createdDate, destroyedDate) <> (ClusterRecord.tupled, ClusterRecord.unapply)
  }

...
//import org.mockito.Mockito.{never, verify, _, when}
...
class LeonardoServiceSpec extends TestKit(ActorSystem("leonardotest")) with FlatSpecLike with Matchers
  ...
  private var cluster1: Cluster = _
...
//    cluster1 = makeCluster(1).copy(status = ClusterStatus.Running, machineConfig = mockMachineConfig1_noWorkers)
  }
  ...
  it should "testIor" in isolatedDbTest {

//    when(gdDAO.resizeCluster(clusterName = ArgumentMatchers.any))

//    leo.maybeResizeCluster(cluster1, Option(mockMachineConfig1_noWorkers))
    val mockMachineConfig1_noWorkers: MachineConfig =  new MachineConfig(numberOfPreemptibleWorkers = Option(0), numberOfWorkers = Option(0))
    val mockMachineConfig2_1Workers: MachineConfig =  new MachineConfig(numberOfPreemptibleWorkers = Option(0), numberOfWorkers = Option(0))

    val updatedNumWorkersAndPreemptiblesOpt = Option(mockMachineConfig1_noWorkers).flatMap { machineConfig =>
      Ior.fromOptions(
        getUpdatedValueIfChanged(mockMachineConfig1_noWorkers.numberOfWorkers, mockMachineConfig2_1Workers.numberOfWorkers),
        getUpdatedValueIfChanged(mockMachineConfig1_noWorkers.numberOfPreemptibleWorkers, mockMachineConfig2_1Workers.numberOfPreemptibleWorkers))
    }

    println(updatedNumWorkersAndPreemptiblesOpt)

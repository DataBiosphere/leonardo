...
class AuthProviderSpec extends FreeSpec with ScalatestRouteTest with Matchers with MockitoSugar with TestComponent with ScalaFutures with OptionValues with GcsPathUtils with TestProxy with BeforeAndAfterAll with CommonTestData{
  ...
  "Leo with an AuthProvider" - {
    "should let you do things if the auth provider says yes" in isolatedDbTest {
      ...
      verify(spyProvider).notifyClusterCreated(any[String], any[WorkbenchEmail], any[GoogleProject], any[ClusterName])(any[ExecutionContext])

      // notification of deletion happens only after it has been fully deleted
      verify(spyProvider, never).notifyClusterDeleted(any[String], any[WorkbenchEmail], any[WorkbenchEmail], any[GoogleProject], any[ClusterName])(any[ExecutionContext])
    }
    ...
    "should not let you do things if the auth provider says no" in isolatedDbTest {
      ...
      verify(spyProvider, Mockito.never).notifyClusterCreated(any[String], any[WorkbenchEmail], any[GoogleProject], any[ClusterName])(any[ExecutionContext])
      verify(spyProvider, Mockito.never).notifyClusterDeleted(any[String], any[WorkbenchEmail], any[WorkbenchEmail], any[GoogleProject], any[ClusterName])(any[ExecutionContext])
    }
    ...
    "should not create a cluster if auth provider notifyClusterCreated returns failure" in isolatedDbTest {
      ...
      verify(spyProvider).notifyClusterCreated(any[String], any[WorkbenchEmail], any[GoogleProject], any[ClusterName])(any[ExecutionContext])
      verify(spyProvider).notifyClusterDeleted(any[String], any[WorkbenchEmail], any[WorkbenchEmail], any[GoogleProject], any[ClusterName])(any[ExecutionContext])
    }

...
/**
  ...
  */
class MockProxyService(proxyConfig: ProxyConfig, gdDAO: DataprocDAO, dbRef: DbReference)
                      (implicit system: ActorSystem, materializer: ActorMaterializer, executionContext: ExecutionContext)
  extends ProxyService(proxyConfig: ProxyConfig, gdDAO: DataprocDAO,  dbRef: DbReference, system.deadLetters) {

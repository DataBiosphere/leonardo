...
class ClusterComponentSpec extends TestComponent with FlatSpecLike with CommonTestData with GcsPathUtils {
  "ClusterComponent" should "list, save, get, and delete" in isolatedDbTest {
    ...
    val c1 = getCluster(1).copy(instances = Set(masterInstance, workerInstance1, workerInstance2),
                                       dataprocInfo = referenceCluster.dataprocInfo.copy(googleId = Some(c1UUID)),
                                       auditInfo = referenceCluster.auditInfo.copy(createdDate = createdDate, dateAccessed = dateAccessed))
    val c1WithErr = getCluster(1).copy(errors = List(err1),
                                              instances = Set(masterInstance, workerInstance1, workerInstance2),
                                              dataprocInfo = referenceCluster.dataprocInfo.copy(googleId = Some(c1UUID)),
                                              auditInfo = referenceCluster.auditInfo.copy(createdDate = createdDate, dateAccessed = dateAccessed))
    val c2 = getCluster(2).copy(status = ClusterStatus.Creating,
                                       auditInfo = referenceCluster.auditInfo.copy(dateAccessed = dateAccessed))
    val c3 = getCluster(3).copy(machineConfig = MachineConfig(Some(3),Some("test-master-machine-type"), Some(500), Some("test-worker-machine-type"), Some(200), Some(2), Some(1)),
    ...
    val c4 = getCluster(4).copy(clusterName = c1.clusterName,
                                       ...
                                       dataprocInfo = referenceCluster.dataprocInfo.copy(operationName = Option(OperationName("op3")), stagingBucket = Some(GcsBucketName("testStagingBucket4"))),
                                       jupyterExtensionUri = Some(jupyterExtensionUri),
                                       jupyterUserScriptUri = Some(jupyterUserScriptUri))
...
//      defaultClientId = None)
  ...
  it should "stop and start a cluster" in isolatedDbTest {
    ...
    val initialCluster = getCluster(1)
//      Cluster(
//        clusterName = name1,
//        googleProject = project,
//        serviceAccountInfo = ServiceAccountInfo(Some(serviceAccountEmail), Some(serviceAccountEmail)),
...
//        defaultClientId = None)
  ...
  it should "get list of clusters to auto freeze" in isolatedDbTest {
    val runningCluster1 = getCluster(1)
//      Cluster(
//      clusterName = name1,
//      googleProject = project,
//      serviceAccountInfo = ServiceAccountInfo(Some(serviceAccountEmail), Some(serviceAccountEmail)),
...
//      defaultClientId = None)
    ...
    val runningCluster2 = getCluster(2)
//      Cluster(
//      clusterName = name2,
//      googleProject = project,
//      serviceAccountInfo = ServiceAccountInfo(Some(serviceAccountEmail), Some(serviceAccountEmail)),
...
//      defaultClientId = None)
    ...
    val stoppedCluster = getCluster(3)
//      Cluster(
//      clusterName = name3,
//      googleProject = project,
//      serviceAccountInfo = ServiceAccountInfo(None, Some(serviceAccountEmail)),

...
import org.broadinstitute.dsde.workbench.google.GoogleUtilities.RetryPredicates.when409
...
class GKEInterpreter[F[_]: Parallel: ContextShift: Timer](
    ...
    for {
      ...
      lastOp <- gkeService
        .pollOperation(
          ...
          config.monitorConfig.createCluster.interval,
          config.monitorConfig.createCluster.maxAttempts
        )
        .compile
        .lastOrError
          ...
          lastOpOpt <- opOpt.traverse { op =>
            gkeService
              .pollOperation(
                ...
                config.monitorConfig.createNodepool.interval,
                config.monitorConfig.createNodepool.maxAttempts
              )
              .compile
              .lastOrError
          }
  ...
  override def createAndPollApp(params: CreateAppParams)(
    ...
    for {
      ...
      // Create KSA
      ksaName = config.galaxyAppConfig.serviceAccountName
      ...
      lastOp <- operationOpt
        .traverse(op =>
          gkeService
            .pollOperation(
              KubernetesOperationId(params.googleProject, dbCluster.location, op.getName),
              config.monitorConfig.deleteCluster.interval,
              config.monitorConfig.deleteCluster.maxAttempts
            )
        )
        .compile
        .lastOrError
          ...
          lastOp <- operationOpt
            .traverse(op =>
              gkeService
                .pollOperation(
                  KubernetesOperationId(params.googleProject, dbCluster.location, op.getName),
                  config.monitorConfig.deleteNodepool.interval,
                  config.monitorConfig.deleteNodepool.maxAttempts
                )
            )
            .compile
            .lastOrError
  ...
  private[algebra] def installNginx(dbCluster: KubernetesCluster,
  ...
  private[algebra] def installGalaxy(appName: AppName,
                                     release: Release,
  ...
  private[algebra] def uninstallGalaxy(dbCluster: KubernetesCluster,
                                       appName: AppName,
                                       release: Release,
                                       namespaceName: NamespaceName,
                                       googleCluster: Cluster)(
  ...
  private[algebra] def getHelmAuthContext(
    googleCluster: Cluster,
    dbCluster: KubernetesCluster,
    namespaceName: NamespaceName
  ...
  private[algebra] def getSecrets(namespace: NamespaceName): F[List[KubernetesSecret]] =
  ...
  private[algebra] def buildGoogleNodepool(nodepool: Nodepool): com.google.container.v1.NodePool = {
  ...
  private[algebra] def buildLegacyGoogleNodepool(
    nodepool: Nodepool
  ...
  private[algebra] def buildGalaxyChartOverrideValuesString(appName: AppName,
                                                            release: Release,
  ...
  private[algebra] def isPodDone(pod: KubernetesPodStatus): Boolean =

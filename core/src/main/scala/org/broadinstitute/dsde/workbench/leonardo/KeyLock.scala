...
object KeyLock {
  def apply[F[_], K <: AnyRef](cache: CaffeineCache[F, K, Semaphore[F]])(implicit
    F: Async[F]
  ): KeyLock[F, K] =
    new KeyLockImpl(cache)

  // A KeyLock implementation backed by a guava cache
  final private class KeyLockImpl[F[_], K <: AnyRef](cache: CaffeineCache[F, K, Semaphore[F]])(implicit
    F: Async[F]
  ) extends KeyLock[F, K] {

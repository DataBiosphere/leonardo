...
object clusterQuery extends TableQuery(new ClusterTable(_)) {
  ...
  def save(saveCluster: SaveCluster)(implicit ec: ExecutionContext): DBIO[Runtime] =
    for {
      ...
      cluster = Runtime.runtimeConfigId.modify(_ => runtimeConfigId.value)(
        saveCluster.cluster
  ...
  def listWithLabels(implicit ec: ExecutionContext, leonardoBaseUrl: LeonardoBaseUrl): DBIO[Seq[Runtime]] =
    clusterLabelPatchQuery.result.map(unmarshalMinimalCluster)

  def listActiveWithLabels(implicit ec: ExecutionContext, leonardoBaseUrl: LeonardoBaseUrl): DBIO[Seq[Runtime]] =
  ...
  def getActiveClusterByName(
    project: GoogleProject,
    name: RuntimeName
  )(implicit ec: ExecutionContext, leonardoBaseUrl: LeonardoBaseUrl): DBIO[Option[Runtime]] =
    fullClusterQueryByUniqueKey(project, name, Some(dummyDate)).result map { recs =>
      unmarshalFullCluster(recs).headOption
    }

  def getDeletingClusterByName(
    project: GoogleProject,
    name: RuntimeName
  )(implicit ec: ExecutionContext, leonardoBaseUrl: LeonardoBaseUrl): DBIO[Option[Runtime]] =
  ...
  def getActiveClusterByNameMinimal(
    project: GoogleProject,
    name: RuntimeName
  )(implicit ec: ExecutionContext, leonardoBaseUrl: LeonardoBaseUrl): DBIO[Option[Runtime]] = {
  ...
  def getClusterById(id: Long)(implicit ec: ExecutionContext, leonardoBaseUrl: LeonardoBaseUrl): DBIO[Option[Runtime]] =
  ...
  private[leonardo] def getIdByUniqueKey(cluster: Runtime)(implicit ec: ExecutionContext,
                                                           leonardoBaseUrl: LeonardoBaseUrl): DBIO[Option[Long]] =
  ...
  private[leonardo] def getIdByUniqueKey(
    ...
    clusterName: RuntimeName,
    destroyedDateOpt: Option[Instant]
  )(implicit ec: ExecutionContext, leonardoBaseUrl: LeonardoBaseUrl): DBIO[Option[Long]] =
  ...
  def getClustersReadyToAutoFreeze(implicit ec: ExecutionContext,
                                   leonardoBaseUrl: LeonardoBaseUrl): DBIO[Seq[Runtime]] = {
  ...
  def clearKernelFoundBusyDateByProjectAndName(
    googleProject: GoogleProject,
    clusterName: RuntimeName,
    dateAccessed: Instant
  )(implicit ec: ExecutionContext, leonardoBaseUrl: LeonardoBaseUrl): DBIO[Int] =
    clusterQuery.getActiveClusterByNameMinimal(googleProject, clusterName) flatMap {
  ...
  )(implicit leonardoBaseUrl: LeonardoBaseUrl): Seq[Runtime] = {
  ...
  private def unmarshalCluster(clusterRecord: ClusterRecord,
                               ...
                               patch: List[PatchRecord])(implicit leonardoBaseUrl: LeonardoBaseUrl): Runtime = {
    ...
    Runtime(
      ...
      Runtime.getProxyUrl(leonardoBaseUrl.asString, project, name, clusterImages, labels),

...
//  def getInternalProxyPaths(apiServerIp: KubernetesApiServerIp): Map[ServiceName, UrlAndPort] =
//    appResources.services.map { service =>
//      service.config.name ->
//        UrlAndPort(
//          new URL(
//            s"${apiServerIp.url}/api/v1/namespaces/${appResources.namespace.name.value}/services/${service.config.name.value}/proxy/"
//          ),

...
class AppComponentSpec extends AnyFlatSpecLike with TestComponent {
  ...
  it should "save basic app" in isolatedDbTest { implicit dbRef =>
  ...
  it should "save complex app" in isolatedDbTest { implicit dbRef =>
    val savedCluster1 = makeKubeCluster(1).save()
    val savedNodepool1 = makeNodepool(1, savedCluster1.id).save()
  ...
  it should "fail to save an app without a nodepool" in isolatedDbTest { implicit dbRef =>
    val savedCluster1 = makeKubeCluster(1).save()

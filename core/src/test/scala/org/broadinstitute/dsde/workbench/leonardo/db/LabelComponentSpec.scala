...
class LabelComponentSpec extends AnyFlatSpecLike with TestComponent with GcsPathUtils {
  ...
  List(LabelResourceType.Runtime, LabelResourceType.PersistentDisk, LabelResourceType.App).foreach { resourceType =>
    it should s"save, get, update, and delete ${resourceType.asString} labels" in isolatedDbTest { implicit dbRef =>
      for {
  ...
  private def makeResource(index: Int, lblType: LabelResourceType)(implicit dbReference: DbReference[IO]): IO[Long] =
    lblType match {

...
class NodepoolComponentSpec extends AnyFlatSpecLike with TestComponent {
  ...
  it should "save, get, delete" in isolatedDbTest { implicit dbRef =>
  ...
  it should "prevent duplicate (clusterId, nodepoolName) nodepools" in isolatedDbTest { implicit dbRef =>
  ...
  it should "update status" in isolatedDbTest { implicit dbRef =>
    val savedCluster1 = makeKubeCluster(1).save()
  ...
  it should "claim nodepool properly" in isolatedDbTest { implicit dbRef =>
    val savedCluster1 = makeKubeCluster(1).save()
  ...
  it should "return a nodepool by user" in isolatedDbTest { implicit dbRef =>
  ...
  it should "return a nodepool by user when the user has 2 apps" in isolatedDbTest { implicit dbRef =>
    val cluster1 = makeKubeCluster(1).save()
  ...
  it should "not return a nodepool by user when it does not exist" in isolatedDbTest { implicit dbRef =>
    val cluster1 = makeKubeCluster(1).save()
  ...
  it should "not return a nodepool in a different project from a user" in isolatedDbTest { implicit dbRef =>
    val cluster1 = makeKubeCluster(1).save()
    val realUser = WorkbenchEmail("real@gmail.com")
    val nodepool1 = makeNodepool(1, cluster1.id).copy(auditInfo = auditInfo.copy(creator = realUser)).save()

...
class KubernetesServiceDbQueriesSpec extends AnyFlatSpecLike with TestComponent {
  ...
  it should "return None when there is no matching app" in isolatedDbTest { implicit dbRef =>
  ...
  it should "get full app" in isolatedDbTest { implicit dbRef =>
    val cluster1 = makeKubeCluster(1).save()
  ...
  it should "get full app with disk and services" in isolatedDbTest { implicit dbRef =>
  ...
  it should "filter clusters and nodepools that have no apps out of list" in isolatedDbTest { implicit dbRef =>
    val cluster1 = makeKubeCluster(1).save()
    val nodepool1 = makeNodepool(1, cluster1.id).save()
  ...
  it should "list deleted apps when includeDeleted is true" in isolatedDbTest { implicit dbRef =>
    val cluster1 = makeKubeCluster(1).save()
  ...
  it should "not list deleted apps when includeDelete is false" in isolatedDbTest { implicit dbRef =>
    val cluster1 = makeKubeCluster(1).save()
    val nodepool1 = makeNodepool(1, cluster1.id).save()
    val app1 = makeApp(1, nodepool1.id).save()
    val app2 = makeApp(2, nodepool1.id).save()
  ...
  it should "save cluster when one doesn't exist for project" in isolatedDbTest { implicit dbRef =>
  ...
  it should "error if cluster exists and is creating on save" in isolatedDbTest { implicit dbRef =>
  ...
  it should "get cluster if exists for project" in isolatedDbTest { implicit dbRef =>
  ...
  it should "save and get errors" in isolatedDbTest { implicit dbRef =>
    val savedCluster1 = makeKubeCluster(1).save()
    val savedNodepool1 = makeNodepool(1, savedCluster1.id).save()
  ...
  it should "correctly identify if theres an operation in progress" in isolatedDbTest { implicit dbRef =>

...
class ClusterImageComponentSpec extends AnyFlatSpecLike with TestComponent {
  ...
  "ClusterImageComponent" should "save and get" in isolatedDbTest { implicit dbRef =>
  ...
  it should "save and get all for cluster" in isolatedDbTest { implicit dbRef =>
    val cluster = makeCluster(1).save()
  ...
  it should "upsert" in isolatedDbTest { implicit dbRef =>
    val cluster = makeCluster(1).save()

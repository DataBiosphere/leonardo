...
class RuntimeServiceDbQueriesSpec extends AnyFlatSpecLike with TestComponent with GcsPathUtils with ScalaFutures {
  ...
  it should "getStatusByName" in isolatedDbTest { implicit dbRef =>
    val r = makeCluster(1)
    val res = for {
  ...
  it should "list runtimes by labels" in isolatedDbTest { implicit dbRef =>
    val res = for {
      start <- testTimer.clock.monotonic(TimeUnit.MILLISECONDS)
      d1 <- makePersistentDisk(Some(DiskName("d1"))).save()
  ...
  it should "list runtimes by project" in isolatedDbTest { implicit dbRef =>
    val res = for {
      start <- testTimer.clock.monotonic(TimeUnit.MILLISECONDS)
      d1 <- makePersistentDisk(Some(DiskName("d1"))).save()
      c1RuntimeConfig = RuntimeConfig.GceWithPdConfig(defaultMachineType, Some(d1.id), bootDiskSize = DiskSize(50))
  ...
  it should "list runtimes including deleted" in isolatedDbTest { implicit dbRef =>
    val res = for {
      start <- testTimer.clock.monotonic(TimeUnit.MILLISECONDS)
      d1 <- makePersistentDisk(Some(DiskName("d1"))).save()
      c1RuntimeConfig = RuntimeConfig.GceWithPdConfig(defaultMachineType, Some(d1.id), bootDiskSize = DiskSize(50))
  ...
  private def toListRuntimeResponse(
    ...
    ListRuntimeResponse2(
      ...
      runtime.auditInfo,
      runtimeConfig,
      Runtime.getProxyUrl(leonaroBaseUrl.asString, runtime.googleProject, runtime.runtimeName, Set.empty, labels),
      runtime.status,
      labels,
      false
    )

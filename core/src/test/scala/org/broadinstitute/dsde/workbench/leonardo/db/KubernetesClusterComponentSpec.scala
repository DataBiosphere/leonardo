...
class KubernetesClusterComponentSpec extends AnyFlatSpecLike with TestComponent {
  ...
  it should "save, get, and delete" in isolatedDbTest { implicit dbRef =>
  ...
  it should "aggregate all sub tables on get, and clean up all tables on delete" in isolatedDbTest { implicit dbRef =>
  ...
  it should "have 1 nodepool when initialized" in isolatedDbTest { implicit dbRef =>
    val savedCluster1 = makeKubeCluster(1).save()
    savedCluster1.nodepools.size shouldBe 1
  }

  it should "prevent duplicate (googleProject, destroyedDate) kubernetes clusters" in isolatedDbTest { implicit dbRef =>
    val cluster1 = makeKubeCluster(1)
  ...
  it should "update async fields" in isolatedDbTest { implicit dbRef =>
    val savedCluster1 = makeKubeCluster(1).save()

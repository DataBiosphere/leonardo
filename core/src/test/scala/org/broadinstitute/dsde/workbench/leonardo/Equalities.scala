...
object Equalities {
  ...
  implicit val subscriberConfigEquality: Equality[SubscriberConfig] = (a: SubscriberConfig, b: Any) => {
    b match {
      case b: SubscriberConfig =>
        a.pathToCredentialJson == b.pathToCredentialJson &&
          a.topicName.toString == b.topicName.toString &&
          a.subscriptionName == b.subscriptionName &&
          a.ackDeadLine == b.ackDeadLine &&
          a.flowControlSettingsConfig == b.flowControlSettingsConfig &&
          a.filter == b.filter
      case _ => false
    }
  }

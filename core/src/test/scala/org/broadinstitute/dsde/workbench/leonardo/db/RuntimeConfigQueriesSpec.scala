...
class RuntimeConfigQueriesSpec extends AnyFlatSpecLike with TestComponent with LeonardoTestSuite {
  "RuntimeConfigQueries" should "save cluster with properties properly" in isolatedDbTest { implicit dbRef =>
  ...
  it should "save gceConfig properly" in isolatedDbTest { implicit dbRef =>
  ...
  it should "save gceWithPdConfig properly" in isolatedDbTest { implicit dbRef =>
    val res = for {
      now <- testTimer.clock.realTime(TimeUnit.MILLISECONDS)

...
class VPCInterpreterSpec extends AnyFlatSpecLike with LeonardoTestSuite {
  ...
  val config = VPCInterpreterConfig(vpcConfig)
  "VPCInterpreter" should "get a subnet from a project label" in {
    val test = new VPCInterpreter(config,
                                  stubResourceService(
  ...
  it should "fail if both labels are not present" in {
    ...
    val test2 = new VPCInterpreter(config,
                                   stubResourceService(
  ...
  it should "create a new subnet if there are no project labels" in {
    val test =
      new VPCInterpreter(config, stubResourceService(Map.empty), FakeGoogleComputeService, new MockComputePollOperation)
  ...
  it should "create firewall rules in the project network" in {
    ...
    val test = new VPCInterpreter(config, stubResourceService(Map.empty), computeService, new MockComputePollOperation)

...
import org.broadinstitute.dsde.workbench.leonardo.db.{kubernetesClusterQuery, nodepoolQuery, DbReference, TestComponent}
...
class GKEInterpreterSpec extends AnyFlatSpecLike with TestComponent with LeonardoTestSuite {
  ...
  val vpcInterp =
    new VPCInterpreter[IO](VPCInterpreterConfig(vpcConfig),
                           FakeGoogleResourceService,
                           FakeGoogleComputeService,
                           new MockComputePollOperation)

  def gkeInterp(implicit db: DbReference[IO]) =
    new GKEInterpreter[IO](GKEInterpreterConfig(testSecurityFilesConfig,
  ...
  "GKEInterpreter" should "create a nodepool with autoscaling" in isolatedDbTest { implicit dbRef =>
  ...
  it should "create secrets properly" in isolatedDbTest { implicit dbRef =>
    val savedCluster1 = makeKubeCluster(1).save()
  ...
  it should "get a helm auth context" in isolatedDbTest { implicit dbRef =>
  ...
  it should "build Galaxy override values string" in isolatedDbTest { implicit dbRef =>
    val savedCluster1 = makeKubeCluster(1).save()
  ...
  it should "check if a pod is done" in isolatedDbTest { implicit dbRef =>
  ...
  it should "deleteAndPollCluster properly" in isolatedDbTest { implicit dbRef =>
    val res = for {
  ...
  it should "deleteAndPollNodepool properly" in isolatedDbTest { implicit dbRef =>
    val res = for {
      savedCluster <- IO(makeKubeCluster(1).save())

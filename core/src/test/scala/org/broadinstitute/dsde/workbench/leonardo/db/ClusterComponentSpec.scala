...
class ClusterComponentSpec extends AnyFlatSpecLike with TestComponent with GcsPathUtils with ScalaFutures {
  "ClusterComponent" should "list, save, get, and delete" in isolatedDbTest { implicit dbRef =>
  ...
  it should "stop and start a cluster" in isolatedDbTest { implicit dbRef =>
  ...
  it should "merge instances" in isolatedDbTest { implicit dbRef =>
  ...
  it should "get list of clusters to auto freeze" in isolatedDbTest { implicit dbRef =>
  ...
  it should "get for dns cache" in isolatedDbTest { implicit dbRef =>
  ...
  it should "update master machine type" in isolatedDbTest { implicit dbRef =>
    val savedCluster1 = makeCluster(1)
  ...
  it should "update master disk size" in isolatedDbTest { implicit dbRef =>
  ...
  it should "list monitored only" in isolatedDbTest { implicit dbRef =>
  ...
  it should "persist custom environment variables" in isolatedDbTest { implicit dbRef =>
  ...
  it should "persist runtimeConfig properly" in isolatedDbTest { implicit dbRef =>
  ...
  it should "persist persistentDiskId foreign key" in isolatedDbTest { implicit dbRef =>
    val res = for {
  ...
  it should "save and get deletedFrom" in isolatedDbTest { implicit dbRef =>
    val res = for {
      savedRuntime <- IO(
        makeCluster(1).save()
      )
      _ <- clusterQuery.updateDeletedFrom(savedRuntime.id, "cryptomining").transaction
      deletedFrom <- clusterQuery.getDeletedFrom(savedRuntime.id).transaction
    } yield {
      deletedFrom shouldBe Some("cryptomining")
    }

...
import org.broadinstitute.dsde.workbench.leonardo.Equalities.diskEq
...
class PersistentDiskComponentSpec extends AnyFlatSpecLike with TestComponent {
  ...
  it should "save and get records" in isolatedDbTest { implicit dbRef =>
  ...
  it should "get by name" in isolatedDbTest { implicit dbRef =>
    val deletedDisk =
      PersistentDisk.destroyedDateLens.modify(_ => Some(Instant.now))(makePersistentDisk(Some(DiskName("d2"))))
  ...
  it should "delete records" in isolatedDbTest { implicit dbRef =>
    val res = for {

      ...
      case (id,
        ...
        PersistentDiskRecord(
          ...
          formattedBy.flatMap {
            ...
            case FormattedBy.GCE | FormattedBy.Custom | FormattedBy.HailBatch => None
          },
          sourceDisk,
          wsmResourceId
        )
  ...
  def isDiskAttached(diskId: DiskId)(implicit ec: ExecutionContext): DBIO[Boolean] =
    for {
      ...
      r <- formattedBy match {
        ...
        case Some(FormattedBy.Galaxy | FormattedBy.Custom | FormattedBy.Cromwell | FormattedBy.HailBatch) =>

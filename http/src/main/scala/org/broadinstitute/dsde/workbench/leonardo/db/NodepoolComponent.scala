...
import org.broadinstitute.dsde.workbench.leonardo.{AutoScalingMax, AutoScalingMin, KubernetesAuditInfo, KubernetesClusterLeoId, Nodepool, NodepoolAutoscaling, NodepoolLeoId, NodepoolStatus, NumNodes}
...
class NodepoolTable(tag: Tag) extends Table[NodepoolRecord](tag, "NODEPOOL") {
  ...
  def * = (id, clusterId, nodepoolName, status, creator, createdDate, destroyedDate, machineType, numNodes, autoScalingEnabled, autoScalingMin, autoScalingMax) <>
...
object nodepoolQuery extends TableQuery(new NodepoolTable(_)) {
  ...
  def getById(id: NodepoolLeoId)
             (implicit ec: ExecutionContext): DBIO[Option[Nodepool]] =
    findByNodePoolIdQuery(id)
      .result
      .headOption
      .map(recOpt => recOpt.map(
        rec => unmarshalNodePool(rec)))

  def getAllForCluster(clusterId: KubernetesClusterLeoId)
                      (implicit ec: ExecutionContext): DBIO[Set[Nodepool]] =
    findByClusterIdQuery(clusterId)
      .result
      .map(rowOpt =>
      rowOpt.map(row => unmarshalNodePool(row))
        .toSet
      )

  def saveForCluster(n: Nodepool)
                    (implicit ec: ExecutionContext): DBIO[Nodepool] = {
    for {

...
class VPCInterpreter[F[_]: Async: Parallel: ContextShift: Logger](
      ...
      (network, subnetwork) <- (networkFromLabel, subnetworkFromLabel) match {
        ...
        case (None, None) =>
          for {
            ...
            _ <- existingNetwork.fold(
              googleComputeService.createNetwork(params.project, buildNetwork(params.project)).void
            )(_ => Async[F].unit)
            existingSubnetwork <- googleComputeService.getSubnetwork(params.project,
                                                                     config.vpcConfig.subnetworkRegion,
                                                                     config.vpcConfig.subnetworkName)
            _ <- existingSubnetwork.fold(
              googleComputeService
                .createSubnetwork(params.project, config.vpcConfig.subnetworkRegion, buildSubnetwork(params.project))
                .void
            )(_ => Async[F].unit)
  ...
  override def setUpProjectFirewalls(
    ...
    for {
      existingHttpsFirewall <- googleComputeService.getFirewallRule(params.project,
                                                                    config.vpcConfig.httpsFirewallRule.name)
      _ <- existingHttpsFirewall.fold(
        googleComputeService
          .addFirewallRule(params.project, buildFirewall(params.project, config.vpcConfig.httpsFirewallRule))
          .void
      )(_ => Async[F].unit)
      defaultNetwork <- googleComputeService.getNetwork(params.project, defaultNetwork)
      _ <- if (defaultNetwork.isDefined) {
        for {
          existingSshFirewall <- googleComputeService.getFirewallRule(params.project,
                                                                      config.vpcConfig.sshFirewallRule.name)
          _ <- if (existingSshFirewall.isEmpty) {
            googleComputeService
              .addFirewallRule(params.project, buildFirewall(params.project, config.vpcConfig.sshFirewallRule))
              .void >>

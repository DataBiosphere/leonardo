...
private[leonardo] object LeoProfile extends MySQLProfile {
  ...
  object mappedColumnImplicits {
    ...
    implicit val formattedByMappedColumnType: BaseColumnType[FormattedBy] =
      MappedColumnType.base[FormattedBy, String](_.asString, s => FormattedBy.withNameInsensitiveOption(s).getOrElse(throw new RuntimeException(s"Unknown formattedBy $s")))

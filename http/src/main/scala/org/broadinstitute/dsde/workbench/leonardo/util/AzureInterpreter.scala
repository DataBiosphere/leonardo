...
import com.azure.resourcemanager.compute.models.{VirtualMachineSizeTypes, PowerState, VirtualMachine}
import org.broadinstitute.dsde.workbench.google2.streamUntilDoneOrTimeout
import org.broadinstitute.dsde.workbench.leonardo.AsyncTaskProcessor.Task
import org.broadinstitute.dsde.workbench.leonardo.dao.{CreateNetworkRequest, CreateDiskRequest, ComputeManagerDao, CreateIpRequest, AzureSubnetName, DeleteVmRequest, ControlledResourceCommonFields, CreateIpRequestData, CloningInstructions, AccessScope, CreateIpResponse, ControlledResourceName, WsmJobId, CreateDiskResponse, ManagedBy, CreateNetworkRequestData, CreateDiskRequestData, CreateVmResult, WsmJobControl, WsmDao, PrivateResourceUser, AzureIpName, CreateVmRequest, CreateNetworkResponse, WsmJobStatus, GetJobResultRequest, AzureNetworkName, CreateVmRequestData, ControlledResourceIamRole, AzureDiskName, ControlledResourceDescription}
import org.broadinstitute.dsde.workbench.leonardo.db.{RuntimeConfigQueries, clusterImageQuery, WsmResourceType, controlledResourceQuery, clusterQuery, clusterErrorQuery, DbReference, persistentDiskQuery}
...
class AzureInterpreter[F[_]](config: AzureInterpretorConfig,
                             monitorConfig: AzureMonitorConfig,
                             asyncTasks: Queue[F, Task[F]],
                             wsmDao: WsmDao[F],
                             azureComputeManager: ComputeManagerDao[F])
                            (implicit val executionContext: ExecutionContext, dbRef: DbReference[F], F: Async[F]) extends AzureAlgebra[F] {
  ...
  override def createAndPollRuntime(msg: CreateAzureRuntimeMessage)(implicit ev: Ask[F, AppContext]): F[Unit] =
    for {
      ...
      runtime <- F.fromOption(runtimeOpt,
       PubsubHandleMessageError.ClusterNotFound(msg.runtimeId, msg))
      ...
      azureRuntimeConfig <- runtimeConfig match {
        ...
        case _ =>
          F.raiseError[RuntimeConfig.AzureVmConfig](
            PubsubHandleMessageError.AzureRuntimeError(msg.runtimeId, ctx.traceId, Some(msg), s"createRuntime in AzureInterp should not get a runtime with a non-azure runtime config")
          )
      }

      pdOpt <- dbRef.inTransaction(persistentDiskQuery.getById(azureRuntimeConfig.persistentDiskId))
      pd <- F.fromOption(pdOpt,
        PubsubHandleMessageError.DiskNotFound(azureRuntimeConfig.persistentDiskId))
      ...
      vmImage <- F.fromOption(vmImageOpt,
        PubsubHandleMessageError.AzureRuntimeError(msg.runtimeId, ctx.traceId, Some(msg), s"createRuntime in AzureInterp should not get a runtime without a CLUSTER_IMAGES table entry that has type RuntimeImageType.AzureVm"))
      ...
      _ <- monitorCreateRuntime(PollRuntimeParams(msg.workspaceId, runtime, wsmWorkspace.azureContext.managedResourceGroupName, createAzureRuntimeResult.jobReport.id))
    } yield ()
  ...
  private def createRuntime(params: CreateAzureRuntimeParams)(implicit ev: Ask[F, AppContext]): F[CreateVmResult] =
    for {
      ...
      vmCommon = getCommonFields(ControlledResourceName(params.runtime.runtimeName.asString), config.vmControlledResourceDesc, params.runtime.auditInfo.creator)
      ...
      _ <- dbRef.inTransaction(controlledResourceQuery.save(
          params.runtime.id,
  ...
  private def monitorCreateRuntime(params: PollRuntimeParams)(implicit ev: Ask[F, AppContext]): F[Unit] = {
    implicit val azureRuntimeCreatingDoneCheckable: DoneCheckable[VirtualMachine] = (v: VirtualMachine) => v.powerState().toString.equals(PowerState.RUNNING.toString)
    implicit val wsmCreateVmDoneCheckable: DoneCheckable[CreateVmResult] = (v: CreateVmResult) => v.jobReport.status.equals(WsmJobStatus.Succeeded)
    for {
      ctx <- ev.ask
      ...
      getRuntime = F.fromOption(getRuntimeOpt,
        PubsubHandleMessageError.AzureRuntimeError(params.runtime.id, ctx.traceId, None, "Could not retrieve vm controlled resource for runtime from DB")
      )

      taskToRun = for {
        // first poll the WSM createVm job for completion
        _ <- streamUntilDoneOrTimeout(getWsmJobResult, monitorConfig.pollStatus.maxAttempts, monitorConfig.pollStatus.interval, s"Wsm createVm job was not completed within ${monitorConfig.pollStatus.maxAttempts} attempts with ${monitorConfig.pollStatus.interval} delay")
        // then poll the azure VM for Running status, retrieving the final azure representation
        azureRuntime <- streamUntilDoneOrTimeout(getRuntime, monitorConfig.pollStatus.maxAttempts, monitorConfig.pollStatus.interval, s"Azure runtime was not running within ${monitorConfig.pollStatus.maxAttempts} attempts with ${monitorConfig.pollStatus.interval} delay")

        // update host ip from azure response and set the runtime to running
        _ <- dbRef.inTransaction(
          clusterQuery.updateClusterHostIp(params.runtime.id, Some(IP(azureRuntime.getPrimaryPublicIPAddress.ipAddress())), ctx.now)
        )
      ...
      _ <- asyncTasks.offer(
        Task(
          ...
          Some(e =>
            dbRef.inTransaction(
                clusterErrorQuery.save(params.runtime.id, RuntimeError(e.getMessage, None, ctx.now, Some(ctx.traceId))) >>
  ...
  override def deleteAndPollRuntime(msg: DeleteAzureRuntimeMessage)(implicit ev: Ask[F, AppContext]): F[Unit] = {
    implicit val azureRuntimeDeletingDoneCheckable: DoneCheckable[Option[VirtualMachine]] = (v: Option[VirtualMachine]) => v.isEmpty
    for {
      ctx <- ev.ask
      ...
      runtime <- F.fromOption(runtimeOpt,
       PubsubHandleMessageError.ClusterNotFound(msg.runtimeId, msg))
      vmControlledResourceOpt <- dbRef.inTransaction(controlledResourceQuery.getResourceTypeForRuntime(msg.runtimeId, WsmResourceType.AzureVm))
      vmControlledResource <- F.fromOption(vmControlledResourceOpt,
       PubsubHandleMessageError.AzureRuntimeError(msg.runtimeId, ctx.traceId, Some(msg), "Could not retrieve vm controlled resource for runtime from DB"))

      params = DeleteAzureRuntimeParams(msg.workspaceId, runtime, vmControlledResource)

      _ <- wsmDao.deleteVm(DeleteVmRequest(
        params.workspaceId,
      ...
      wsmWorkspace <- wsmDao.getWorkspace(msg.workspaceId)
      getDeleteResult = azureComputeManager.getAzureVm(runtime.runtimeName, wsmWorkspace.azureContext.managedResourceGroupName)

      taskToRun = for {
        _ <- streamUntilDoneOrTimeout(getDeleteResult, monitorConfig.pollStatus.maxAttempts, monitorConfig.pollStatus.interval, s"Azure vm still exists after ${monitorConfig.pollStatus.maxAttempts} attempts with ${monitorConfig.pollStatus.interval} delay")
  ...
  private def createIp(params: CreateAzureRuntimeParams)(implicit ev: Ask[F, AppContext]): F[CreateIpResponse] = {
    val common = getCommonFields(ControlledResourceName(uniqueName(config.ipNamePrefix)), config.ipControlledResourceDesc, params.runtime.auditInfo.creator)
  ...
  private def createDisk(params: CreateAzureRuntimeParams)(implicit ev: Ask[F, AppContext]): F[CreateDiskResponse] = {
    val common = getCommonFields(ControlledResourceName(params.pd.name.value), config.diskControlledResourceDesc, params.runtime.auditInfo.creator)
  ...
  private def createNetwork(params: CreateAzureRuntimeParams)(implicit ev: Ask[F, AppContext]): F[CreateNetworkResponse] = {
    val networkName = uniqueName(config.networkNamePrefix)
    val common = getCommonFields(ControlledResourceName(networkName), config.networkControlledResourceDesc, params.runtime.auditInfo.creator)

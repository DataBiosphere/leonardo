...
object extensionQuery extends TableQuery(new ExtensionTable(_)) {
  ...
  def unmarshallExtensions(extList: List[ExtensionRecord]): Option[UserJupyterExtensionConfig] =
    if (extList.isEmpty) {
      ...
      val nbExtension =
        extList.filter(_.extensionType == ExtensionType.NBExtension.toString).map(x => x.name -> x.value).toMap
      val serverExtension =
        extList.filter(_.extensionType == ExtensionType.ServerExtension.toString).map(x => x.name -> x.value).toMap
      val combinedExtension =
        extList.filter(_.extensionType == ExtensionType.CombinedExtension.toString).map(x => x.name -> x.value).toMap
      val labExtension =
        extList.filter(_.extensionType == ExtensionType.LabExtension.toString).map(x => x.name -> x.value).toMap

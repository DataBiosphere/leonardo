...
class AppTable(tag: Tag) extends Table[AppRecord](tag, "APP") {
  ...
  def appConfigId = column[AppConfigId]("appConfigId")
...
object appQuery extends TableQuery(new AppTable(_)) {
  ...
  def save(saveApp: SaveApp)(implicit ec: ExecutionContext): DBIO[App] = {
    ...
    for {
      ...
      appConfigId <- AppConfigQueries.insertAppConfig(saveApp.appConfig, saveApp.app.auditInfo.dateAccessed)
    ...
    } yield saveApp.app.copy(id = appId,
                             appResources = AppResources(namespace, services, ksaOpt),
                             appConfigId = appConfigId)
  }
...
//      .update(chart)
  ...
  // TODO use AppConfigQuerry for disk actions
...
case class SaveApp(app: App, appConfig: AppConfig)

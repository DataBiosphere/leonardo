...
import org.broadinstitute.dsde.workbench.leonardo.http.service.{CreateClusterAPIResponse, GetClusterResponse, ListClusterResponse, UpdateClusterResponse}
...
object LeoRoutesJsonCodec extends DefaultJsonProtocol {
  ...
  implicit val dataprocConfigDecoder: Decoder[RuntimeConfigRequest.DataprocConfig] = Decoder.instance {
    c =>
      for {
        ...
        _ <- if(masterMachineType.nonEmpty && masterMachineType.exists(_.isEmpty)) Left(emptyMasterMachineType) else Right(())
        masterDiskSize <- c.downField("masterDiskSize").as[Option[Int]].flatMap(x => if(x.exists(_ < 0)) Left(negativeNumberDecodingFailure) else Right(x))
        ...
        res <- numberOfWorkersInput match {
          ...
          case Some(x) => Right(RuntimeConfigRequest.DataprocConfig(Some(x), masterMachineType, masterDiskSize, workerMachineType, workerDiskSize, numberOfWorkerLocalSSDs, numberOfPreemptibleWorkers))
          case None =>
            Right(RuntimeConfigRequest.DataprocConfig(None, masterMachineType, masterDiskSize, workerMachineType, workerDiskSize, numberOfWorkerLocalSSDs, numberOfPreemptibleWorkers))
        }
      } yield res
  }
  ...
  implicit val runtimeConfigDecoder: Decoder[model.RuntimeConfigRequest] = Decoder.instance {
    x =>

  ...
  override def deleteDisk(userInfo: UserInfo, googleProject: GoogleProject, diskName: DiskName)(implicit
    ...
    for {
      ...
      _ <-
        ...
        else F.raiseError[Unit](DiskCannotBeDeletedException(disk.id, disk.status.toString, cloudContext, ctx.traceId))
...
case class DiskCannotBeDeletedException(id: DiskId, status: String, cloudContext: CloudContext, traceId: TraceId)
    extends LeoException(

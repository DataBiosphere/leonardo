...
case class NoGoogleProjectNumberException(googleProject: GoogleProject)
    extends LeoException(s"Project number could not be found for Google project $googleProject", StatusCodes.NotFound)
...
class BucketHelper(
  ...
  def createInitBucket(googleProject: GoogleProject,
                       ...
                       serviceAccountInfo: ServiceAccountInfo): Future[GcsBucketName] =
    for {
      ...
      _ <- retryUntilSuccessOrTimeout(
        failureLogMessage = s"Init bucket creation failed for Google project '$googleProject'"
  ...
  )(implicit ev: ApplicativeAsk[IO, TraceId]): Future[GcsBucketName] =
    for {
      ...
      bucketSAs <- getBucketSAs(googleProject, serviceAccountInfo)
      providerReaders <- serviceAccountProvider
        .listUsersStagingBucketReaders(userEmail)
        .map(_.map(userEntity))
        .unsafeToFuture()
      providerGroups <- serviceAccountProvider
        .listGroupsStagingBucketReaders(userEmail)
        .map(_.map(groupEntity))
        .unsafeToFuture()
      ...
      // the googleStorageDAO.createBucket method
      _ <- retryUntilSuccessOrTimeout(
        failureLogMessage = s"Staging bucket creation failed for Google project '$googleProject'"
      )(30 seconds, 5 minutes) { () =>
   ...
   */
  def updateUserBucket(bucketName: GcsBucketName,
                       googleProject: GoogleProject,
  ...
  private def getConvenienceEntities(
    googleProject: GoogleProject,
    googleProjectNumberOpt: Option[Long]
  ): Future[(GcsEntity, GcsEntity, GcsEntity)] = {
  ...
  private def setBucketAcls(bucketName: GcsBucketName,
                            readers: List[GcsEntity],
                            owners: List[GcsEntity]): Future[Unit] = {
  ...
  private def getBucketSAs(googleProject: GoogleProject,
                           serviceAccountInfo: ServiceAccountInfo): Future[List[GcsEntity]] = {
    // cluster SA orElse compute engine default SA
    val clusterOrComputeDefault = OptionT.fromOption[Future](serviceAccountInfo.clusterServiceAccount) orElse OptionT(
      googleComputeDAO.getComputeEngineDefaultServiceAccount(googleProject)
    )

...
if [ ! -z "$JUPYTER_DOCKER_IMAGE" ] ; then
  ...
  if [ ! -z "$USER_SCRIPT_URI" ] ; then
    log 'Running user script [$USER_SCRIPT_URI]...'
    ...
    EXIT_CODE=0
    docker exec --privileged -u root -e PIP_TARGET=${ROOT_USER_PIP_DIR} ${JUPYTER_SERVER_NAME} ${JUPYTER_HOME}/${USER_SCRIPT} &> us_output.txt || EXIT_CODE=$?
  ...
  if [ ! -z "$START_USER_SCRIPT_URI" ] ; then
    ...
    EXIT_CODE=0
    docker exec --privileged -u root -e PIP_TARGET=${ROOT_USER_PIP_DIR} ${JUPYTER_SERVER_NAME} ${JUPYTER_HOME}/${START_USER_SCRIPT} &> start_output.txt || EXIT_CODE=$?
    if [ $EXIT_CODE -ne 0 ]; then

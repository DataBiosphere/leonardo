...
import org.broadinstitute.dsde.workbench.leonardo.auth.{AllowlistAuthProvider, SamAuthProvider}
...
class AppServiceInterpTest extends AnyFlatSpec with AppServiceInterpSpec with LeonardoTestSuite with TestComponent {
  ...
  it should "V1 GCP - deleteAllAppsRecords, log an error but not fail when cleaning up already deleted app" in isolatedDbTest {
    ...
    // Manually set app status to Deleted
    val appResult = dbFutureValue {
      appQuery.getAppByName(appName)
    }.get
    dbFutureValue {
      appQuery.updateStatus(appResult.id, AppStatus.Deleted)
    }
    ...
    when(mockAuthProvider.getActions(any, any)(any, any)).thenAnswer { invocation =>
      ...
      if (arg == appResult.samResourceId) IO.pure(List.empty)
    }

...
import org.broadinstitute.dsde.workbench.leonardo.model.{AuditInfo, Cluster, ClusterImage, ClusterInternalId, DataprocInfo, UserJupyterExtensionConfig}
...
object RoutesTestJsonSupport extends DefaultJsonProtocol{
  ...
  implicit val listClusterResponseReaderJsonReader: RootJsonReader[ListClusterResponse] = (json: JsValue) => json match {
    case JsObject(fields: Map[String, JsValue]) =>
      ListClusterResponse(
        fields.getOrElse("id", JsNull).convertTo[Long],
        fields.getOrElse("internalId", JsNull).convertTo[ClusterInternalId],

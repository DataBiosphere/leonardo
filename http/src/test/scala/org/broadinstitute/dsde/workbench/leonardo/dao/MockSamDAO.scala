...
class MockSamDAO extends SamDAO[IO] {
  ...
  override def hasResourcePermission(resourceId: ValueObject, action: String, resourceTypeName: ResourceTypeName, authHeader: Authorization)(implicit ev: ApplicativeAsk[IO, TraceId]): IO[Boolean] = {
      resourceTypeName match {
        case ResourceTypeName.BillingProject =>
          val res = billingProjects.get((resourceId.asInstanceOf[GoogleProject], authHeader)) //look it up: Option[Set]
        ...
        case ResourceTypeName.NotebookCluster =>
          val res = notebookClusters.get((resourceId.asInstanceOf[ClusterInternalId], authHeader)) //look it up: Option[Set]
            .map( _.contains(action) ) //open the option to peek the set: Option[Bool]
            .getOrElse(false) //unpack the resulting option and handle the project never having existed
          IO.pure(res)
      }
  ...
  override def getResourcePolicies[A](authHeader: Authorization, resourseTypeName: ResourceTypeName)(implicit decoder: EntityDecoder[IO, List[A]], ev: ApplicativeAsk[IO, TraceId]): IO[List[A]] = {
    resourseTypeName match {
      case ResourceTypeName.NotebookCluster => IO.pure(clusterCreators.get(authHeader).map(_.toList).getOrElse(List.empty)).map(_.asInstanceOf[List[A]])
      case ResourceTypeName.BillingProject => IO.pure(projectOwners.get(authHeader).map(_.toList).getOrElse(List.empty)).map(_.asInstanceOf[List[A]])
    }
  }
  ...
  override def createClusterResource(internalId: ClusterInternalId, creatorEmail: WorkbenchEmail, googleProject: GoogleProject, clusterName: ClusterName)(implicit ev: ApplicativeAsk[IO, TraceId]): IO[Unit] = {
    IO(notebookClusters += (internalId, userEmailToAuthorization(creatorEmail)) -> Set("status", "connect", "sync", "delete", "read_policies"))
  }

  override def deleteClusterResource(internalId: ClusterInternalId, userEmail: WorkbenchEmail, creatorEmail: WorkbenchEmail, googleProject: GoogleProject, clusterName: ClusterName)
  ...
  override def getCachedPetAccessToken(userEmail: WorkbenchEmail, googleProject: GoogleProject)
                                      (implicit ev: ApplicativeAsk[IO, TraceId]): IO[Option[String]] = IO.pure(Some("token"))

  override def getStatus(implicit ev: ApplicativeAsk[IO, TraceId]): IO[StatusCheckResponse] = IO.pure(StatusCheckResponse(true, Map.empty))
}

object MockSamDAO {
  def userEmailToAuthorization(workbenchEmail: WorkbenchEmail): Authorization = Authorization(Credentials.Token(AuthScheme.Bearer, s"TokenFor${workbenchEmail}"))
}
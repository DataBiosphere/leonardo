...
class ProxyRoutesSpec
  ...
  override def proxyConfig: ProxyConfig = Config.proxyConfig
  ...
  def beforeEachTest(proxyService: ProxyService) = {
  ...
  it should "listen on /proxy/{project}/{name}" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/$googleProject/$clusterName").addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
      handled shouldBe true
      status shouldEqual StatusCodes.OK
      validateCors()
    }
    Get(s"/proxy/$googleProject/$clusterName/foo").addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
      handled shouldBe true
      status shouldEqual StatusCodes.OK
      validateCors()
    }
    Get(s"/proxy/").addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
    ...
    Get(s"/api/proxy").addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
  ...
  it should "404 for non-existent runtimes" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    val newName = "aDifferentClusterName"
    // should 404 since the internal id cannot be looked up
    Get(s"/proxy/$googleProject/$newName").addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
  ...
  it should "set CORS headers in runtime proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/$googleProject/$clusterName")
      .addHeader(Cookie(tokenCookie))
      .addHeader(Origin("http://example.com")) ~> httpRoutes(ps).route ~> check {
      handled shouldBe true
      status shouldEqual StatusCodes.OK
  ...
  it should "reject non-cookied runtime proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/$googleProject/$clusterName") ~> httpRoutes(ps).route ~> check {
      handled shouldBe true
      status shouldEqual StatusCodes.Unauthorized
    }
  }

  it should "404 when using a non-white-listed user in a runtime proxy request" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/$googleProject/$clusterName")
      .addHeader(Cookie(unauthorizedTokenCookie)) ~> httpRoutes(ps).route ~> check {
      status shouldEqual StatusCodes.NotFound
    }
  }

  it should "401 when using an expired token in a runtime proxy request" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/$googleProject/$clusterName").addHeader(Cookie(expiredTokenCookie)) ~> httpRoutes(ps).route ~> check {
      status shouldEqual StatusCodes.Unauthorized
    }
  }

  it should s"pass through paths in runtime proxy requests" in isolatedDbTest { implicit dbRef =>
  ...
  it should "pass through query string params in runtime proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/$googleProject/$clusterName").addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
    ...
    Get(s"/proxy/$googleProject/$clusterName?foo=bar&baz=biz")
      .addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
      responseAs[Data].qs shouldEqual Some("foo=bar&baz=biz")
    }
  }

  it should "pass through encoded query string params in runtime proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
  ...
  it should "pass through http methods in runtime proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/$googleProject/$clusterName").addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
      responseAs[Data].method shouldBe "GET"
    }
    Post(s"/proxy/$googleProject/$clusterName").addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
      responseAs[Data].method shouldBe "POST"
    }
    Put(s"/proxy/$googleProject/$clusterName").addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
      responseAs[Data].method shouldBe "PUT"
    }
  }

  it should "pass through headers in runtime proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/$googleProject/$clusterName")
      .addHeader(Cookie(tokenCookie))
      .addHeader(RawHeader("foo", "bar"))
      .addHeader(RawHeader("baz", "biz")) ~> httpRoutes(ps).route ~> check {
      responseAs[Data].headers.toList should contain allElementsOf Map("foo" -> "bar", "baz" -> "biz").toList
    }
  }

  it should "remove utf-8'' from content-disposition header filenames" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
  ...
  it should "proxy websockets" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    val bindingFuture = Http().bindAndHandle(httpRoutes(ps).route, "0.0.0.0", 9000)
  ...
  it should "create a new terminal when trying to access a terminal that was not created yet" in isolatedDbTest {
    implicit dbRef =>
  ...
  "app proxy routes" should "listen on /proxy/google/v1/apps/{project}/{name}/{service}" in isolatedDbTest {
    implicit dbRef =>
      val ps = proxyService
      beforeEachTest(ps)
  ...
  it should "404 for non-existent apps" in isolatedDbTest { implicit dbRef =>
  ...
  it should "set CORS headers in app proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/google/v1/apps/$googleProject/$appName/$serviceName")
      .addHeader(Cookie(tokenCookie))
  ...
  it should "reject non-cookied requests in app proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/google/v1/apps/$googleProject/$appName/$serviceName") ~> httpRoutes(ps).route ~> check {
      handled shouldBe true
      status shouldEqual StatusCodes.Unauthorized
    }
  }

  it should s"401 when using an expired token in app proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/google/v1/apps/$googleProject/$appName/$serviceName")
      .addHeader(Cookie(expiredTokenCookie)) ~> httpRoutes(ps).route ~> check {
      status shouldEqual StatusCodes.Unauthorized
    }
  }

  it should s"pass through paths in app proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/google/v1/apps/$googleProject/$appName/$serviceName")
      .addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
  ...
  it should "pass through query string params in app proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
  ...
  it should "pass through encoded query string params in app proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
  ...
  it should "pass through http methods in app proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/google/v1/apps/$googleProject/$appName/$serviceName")
      .addHeader(Cookie(tokenCookie)) ~> httpRoutes(ps).route ~> check {
  ...
  it should "pass through headers in app proxy requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/google/v1/apps/$googleProject/$appName/$serviceName")
      .addHeader(Cookie(tokenCookie))
  ...
  "setCookie" should "set a cookie given a valid Authorization header" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    // cache should not initially contain the token
    ps.googleTokenCache.asMap().containsKey(tokenCookie.value) shouldBe false
    ...
    ps.googleTokenCache.asMap().containsKey(tokenCookie.value) shouldBe true
  }

  it should "handle preflight OPTIONS requests" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
  ...
  it should "unset the cookie when not given an Authorization header" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/$googleProject/$clusterName/setCookie")
      .addHeader(Origin("http://example.com")) ~> httpRoutes(ps).route ~> check {
  ...
  it should "401 when using an expired token" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    Get(s"/proxy/$googleProject/$clusterName")
  ...
  "invalidateToken" should "remove a token from cache" in isolatedDbTest { implicit dbRef =>
    val ps = proxyService
    beforeEachTest(ps)
    // cache should not initially contain the token
    ps.googleTokenCache.asMap().containsKey(tokenCookie.value) shouldBe false
  ...
  def createHttpRoute(jupyterDAO: JupyterDAO[IO])(implicit dbRef: DbReference[IO]): HttpRoutes = {
    val proxyService =
      new MockProxyService(proxyConfig,
                           jupyterDAO,
                           whitelistAuthProvider,
